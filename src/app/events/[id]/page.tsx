    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300 mb-8">{error || 'The event you are looking for does not exist.'}</p>
          <Link href="/events" className="bg-gradient-to-r from-pink-500 to-purple-600 text-white px-6 py-3 rounded-lg hover:from-pink-600 hover:to-purple-700 transition-all duration-200">
            Back to Events
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900">
      ...
      <div className="relative">
        <div className="relative h-[60vh] overflow-hidden bg-gradient-to-br from-pink-500 to-purple-600">
          {/* Debug info */}
          {process.env.NODE_ENV === 'development' && (
            <div className="absolute top-4 left-4 z-50 bg-white/90 text-black p-2 rounded text-xs">
              Event Image: {event.image ? 'Yes' : 'No'} |
              Title: {event.title}
            </div>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-xl shadow-sm border border-white/20 p-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {event.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="bg-gradient-to-r from-pink-500/20 to-purple-600/20 text-pink-200 border border-pink-400/30 px-3 py-1 rounded-full text-sm font-medium backdrop-blur-sm"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            {event.organizer && (
              <div className="bg-white/10 backdrop-blur-sm rounded-xl shadow-sm border border-white/20 p-8">
                <h3 className="text-lg font-semibold text-white mb-4">Event Organizer</h3>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-gradient-to-r from-pink-500/20 to-purple-600/20 backdrop-blur-sm border border-purple-400/30 rounded-full flex items-center justify-center">
                ...
                <div className="flex flex-wrap gap-4">
                  {event.socialLinks?.website && (
                    <a
                      ...
                      className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white px-4 py-2 rounded-lg transition-colors"
                    >
                  ...
                  {event.socialLinks?.facebook && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center space-x-2 bg-blue-500/20 hover:bg-blue-500/30 backdrop-blur-sm border border-blue-400/30 text-blue-200 px-4 py-2 rounded-lg transition-colors"
                    >
                  ...
                  {event.socialLinks?.twitter && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center space-x-2 bg-sky-500/20 hover:bg-sky-500/30 backdrop-blur-sm border border-sky-400/30 text-sky-200 px-4 py-2 rounded-lg transition-colors"
                    >
                  ...
                  {event.socialLinks?.instagram && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center space-x-2 bg-pink-500/20 hover:bg-pink-500/30 backdrop-blur-sm border border-pink-400/30 text-pink-200 px-4 py-2 rounded-lg transition-colors"
                    >
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8">
              {/* Event Info Card */}
              <div className="bg-white/10 backdrop-blur-sm rounded-xl shadow-sm border border-white/20 p-6 mb-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      {event.time && (
                        <p className="text-gray-300">at {event.time}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      {event.ticketTiers ? (
                        <>
                          <p className="font-semibold text-white">
                            {Math.max(0, (event.ticketTiers?.reduce((sum, t: any) => sum + (t.remaining ?? Math.max(0, (t.capacity || 0) - (t.sold || 0))), 0)))} spots available
                          </p>
                          <p className="text-gray-300">
                            {event.ticketTiers?.reduce((sum, t: any) => sum + (t.sold || 0), 0)} of {event.ticketTiers?.reduce((sum, t: any) => sum + (t.capacity || 0), 0)} sold
                          </p>
                        </>
                      ) : (
                        <>
                          <p className="font-semibold text-white">
                            {Math.max(0, (event.capacity || 0) - (event.soldTickets || 0))} spots available
                          </p>
                          <p className="text-gray-300">
                ...
                <div className="mt-6">
                  <div className="flex justify-between text-sm text-gray-300 mb-2">
                    ...
                    <span>{(() => {
                      const totalCap = event.ticketTiers?.reduce((s: number, t: any) => s + (t.capacity || 0), 0) || (event.capacity || 0)
                      const sold = event.ticketTiers?.reduce((s: number, t: any) => s + (t.sold || 0), 0) || (event.soldTickets || 0)
                      return totalCap > 0 ? Math.round((sold / totalCap) * 100) : 0
                    })()}%</span>
                  </div>
                  <div className="w-full bg-gray-700/50 rounded-full h-2 backdrop-blur-sm border border-gray-600/30">
                    <div
              ...
              {/* Ticket Selection */}
              <div className="bg-white/10 backdrop-blur-sm rounded-xl shadow-sm border border-white/20 p-6" id="ticket-section">
                <h3 className="text-lg font-semibold text-white mb-6">Select Tickets</h3>

                {event.ticketTiers ? (
                  <div className="space-y-4">
                    {event.ticketTiers.map((tier) => (
                      <div key={tier.id} className="border border-white/20 backdrop-blur-sm bg-white/5 rounded-lg p-4">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h4 className="font-semibold text-white">{tier.name}</h4>
                            <p className="text-sm text-gray-300">{tier.description}</p>
                          </div>
                          <p className="font-bold text-lg text-white">${tier.price}</p>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <div className="flex items-center space-x-3">
                            <button
                              ...
                              className="w-8 h-8 rounded-full border border-white/20 bg-white/10 backdrop-blur-sm flex items-center justify-center hover:bg-white/20 text-white"
                          ...
                          <p className="text-sm text-gray-300">
                            {typeof (tier as any).remaining === 'number'
                              ? Math.max(0, (tier as any).remaining)
                              : Math.max(0, (tier.capacity || 0) - ((tier as any).sold || 0))} remaining
                          </p>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="border border-white/20 backdrop-blur-sm bg-white/5 rounded-lg p-4">
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <h4 className="font-semibold text-white">General Admission</h4>
                        <p className="text-sm text-gray-300">Access to the event</p>
                      </div>
                      <p className="font-bold text-lg text-white">${event.price}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm text-gray-300">
                        {(() => {
                          if (event.ticketTiers) {
                            const tiers = (event.ticketTiers as any[]) || []
                            return Math.max(0, tiers.reduce((s: number, t: any) => s + ((t.remaining ?? Math.max(0, (t.capacity || 0) - (t.sold || 0)))), 0))
                          }
                ...
                <div className="mt-6 pt-6 border-t border-white/20">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white py-3 px-4 rounded-lg font-semibold hover:from-pink-600 hover:to-purple-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 disabled:hover:scale-100"
                  >
              ...
              <div className="mt-6 space-y-3">
                <button
                  ...
                  className="w-full bg-white/10 backdrop-blur-sm border border-white/20 text-white py-2 px-4 rounded-lg font-medium hover:bg-white/20 transition-colors flex items-center justify-center"
                >
